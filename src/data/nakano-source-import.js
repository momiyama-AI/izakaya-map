...
const defaultSourceCsvPath = path.join(
  rootDir,
  "data",
  "nakano-izakaya-source-backed-prices-2026-06-10.csv",
);
const defaultActor = "nakano-web-import";
...
function normalizeStoreFields(store) {
  return {
    areaId: store.areaId,
    name: store.name,
    address: store.address,
    stationExit: store.stationExit,
...
function storeNeedsUpdate(existingStore, nextStore) {
  const existing = normalizeStoreFields(existingStore);
  const next = normalizeStoreFields(nextStore);
  return Object.keys(next).some((key) => existing[key] !== next[key]);
}
...
function buildStore(storeKey, rows, actor = defaultActor) {
  const first = rows[0];
...
function buildDrinkPrice(storeId, storeKey, row, actor = defaultActor) {
...
async function importNakanoSourceBackedPrices(database, options = {}) {
  const sourceCsvPath = options.sourceCsvPath || process.env.NAKANO_SOURCE_CSV || defaultSourceCsvPath;
  const actor = options.actor || process.env.IMPORT_ACTOR || defaultActor;
  ...
  for (const [storeKey, storeRows] of grouped.entries()) {
    const nextStore = buildStore(storeKey, storeRows, actor);
    ...
    let store;
    if (existingStore && storeNeedsUpdate(existingStore, nextStore)) {
      store = await database.updateStore(existingStore.id, nextStore, actor);
      result.storesUpdated += 1;
    } else if (existingStore) {
      store = existingStore;
      result.storesSkipped += 1;
    } else {
      store = await database.insertStore(nextStore, actor);
    ...
    for (const row of storeRows) {
      const price = buildDrinkPrice(store.id, storeKey, row, actor);
...
module.exports = {
  importNakanoSourceBackedPrices,
  parseCsv,
};
